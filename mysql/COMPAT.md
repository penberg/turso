...
| `DATE_FORMAT(x, fmt)`                   | 🚧     | Lowered to `strftime()` with the format translated: `%Y %m %d %H` pass through, `%i`→`%M` (minutes), `%s`→`%S` (seconds), `%j` (day of year) and `%w` (weekday number) pass through (same meaning in strftime), `%U` (Sunday-first week, mode 0) passes through, `%v` (Monday-first ISO week, mode 3)→`%V`, `%T`→`%H:%M:%S`, `%%` literal, other characters copied. The format must be a string literal. Specifiers without a strftime equivalent (`%M` month name, `%h` 12-hour, `%p`, `%W` weekday name, `%u`/`%V` other week modes, …) are rejected rather than silently mistranslated (notably `%W`, which is the week *number* in strftime). |
