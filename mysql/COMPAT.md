...
| Version-gated executable comments (`/*!##### ... */`) | 🚧 | A `/*! ... */` comment (optionally version-gated, `/*!40101 ... */`) has its contents **executed as SQL** when the gate is at most the reported server version (`8.0` → `80000`), rather than discarded like a plain `/* ... */`. So `/*!40101 SET NAMES utf8mb4 */` runs the `SET`, `SELECT /*!40001 SQL_NO_CACHE */ 1` keeps the hint, and a wholly wrapped statement (`/*!40100 CREATE TABLE ... */`) is no longer empty input — the patterns mysqldump output and WordPress/plugin migrations rely on. A gate above the server version is skipped like an ordinary comment. The markers are recognized lexically, so an identical sequence inside a string literal is untouched. Two limitations: a statement that the server dispatches by text prefix *before* parsing (`SET NAMES`, `SHOW`) is not unwrapped, so a top-level `/*!… SET NAMES … */` still falls through to the parser; and a nested plain `/* ... */` comment **inside** an executable comment is not supported (pathological; absent from real dumps). |
