...
|-----------|--------|---------|
| `NOT NULL` / `NULL`         | ✅ | A `NOT NULL` column with no explicit `DEFAULT` is given MySQL's implicit type default (`0` for numeric types, `''` for string/binary types) so a row that omits it still inserts — matching MySQL's default non-strict `sql_mode`, the mode WordPress runs under (a strict `sql_mode` would instead reject the row). `AUTO_INCREMENT` and `PRIMARY KEY` columns are excluded (the engine generates / rowid-handles their values), whether the key is declared inline or at table level — so a primary-key column reports a `NULL` `SHOW COLUMNS` `Default`, matching MySQL. Date/time, `ENUM`/`SET`, `JSON`, and unrecognized types stay strictly `NOT NULL` (their MySQL defaults — the zero date, the first enum member, … — don't map cleanly). For a **non-key** `NOT NULL` column the synthesized default surfaces as its `Default` in `SHOW COLUMNS`/`DESCRIBE` (`0`/`''`), whereas MySQL reports `NULL` there — a minor introspection divergence. |
...
| HANDLER                                | ❌     |         |
| IMPORT TABLE                           | ❌     |         |
| INSERT ... VALUES (basic)              | ✅     | Multi-row `VALUES` supported. The `DEFAULT` keyword may stand in for any value (`VALUES (1, DEFAULT)`), inserting that column's declared default. The empty form `INSERT INTO t () VALUES ()` (and the column-list-less `INSERT INTO t VALUES ()`) inserts one all-defaults row, lowered to the engine's `DEFAULT VALUES`; the multi-row form `VALUES (), ()` (no single-statement engine equivalent) expands to one `DEFAULT VALUES` insert per row. (`DEFAULT` is honored only in `INSERT ... VALUES`, not in `UPDATE ... SET`, and the `DEFAULT(col)` function form is unsupported.) |
| INSERT ... SET                         | ✅     | The `INSERT [INTO] t SET col = expr, ...` assignment form, built as the equivalent `(cols) VALUES (exprs)`. `col = DEFAULT` inserts the column's declared default (the engine's `DEFAULT` keyword, honored in the lowered `VALUES`), the same as the `VALUES` form and as omitting the column. |
| INSERT ... SELECT                      | ✅     | `INSERT [INTO] t [(cols)] SELECT ...`; the query runs through the same SELECT subset, evaluated by the engine. A trailing `ON DUPLICATE KEY UPDATE` carries through (the query's rows are the would-be-inserted values), the same upsert as the `VALUES`/`SET` forms — including `VALUES(col)` in the assignment. |
...
| `SELECT ROW_COUNT()`           | 🚧     | Returns the affected-row count of the previous statement on the connection: the rows changed by an `INSERT`/`UPDATE`/`DELETE`, `0` after a statement that changes no rows (DDL, transaction control, a no-match DML), and `-1` after a statement that returns a result set or fails. The engine has no such function, so the server answers a standalone `SELECT ROW_COUNT()` from the affected-row count it tracks per statement (the same value its `OK` packet carries). Only the bare form is recognized — an inline use (`ROW_COUNT() + 1`) is still rejected — and the count inherits the front-end's affected-row divergences (a `REPLACE`/`INSERT ... ON DUPLICATE KEY` that overwrites a row reports 1 rather than MySQL's 2, and an `UPDATE` to an unchanged value reports 1 rather than 0). |
...
| Comparison chaining (`a = b = c`)      | ✅     | The comparison operators (`= <> < <= > >= <=>`, `IN`, `LIKE`, `REGEXP`, `BETWEEN`, and the `IS` postfix) share one precedence level and **chain left-to-right**, as in MySQL: `1 = 1 = 1` is `(1 = 1) = 1`, `3 > 2 > 1` is `(3 > 2) > 1`, and a predicate result feeds the next (`x IN (1) = 1`, `a BETWEEN 0 AND 2 = 1`). One divergence: MySQL gives `BETWEEN` a special non-associative treatment in pathological chains (`a = b BETWEEN c AND d` parses there as `a = (b BETWEEN c AND d)`, and `a BETWEEN b AND c BETWEEN d AND e` as `a BETWEEN b AND (c BETWEEN d AND e)`), whereas the front-end chains those left-associatively — only matters for forms real queries always parenthesize. |
| Row values `(a, b) = (1, 2)` / `IN`    | ✅     | A parenthesized comma-separated list is a row-value tuple; row comparisons (`=`, `<>`, `<`, `<=`, `>`, `>=`, `IN`, `NOT IN`) compare element-wise (ordering is lexicographic), evaluated by the engine's row-value support. Used for compound-key lookups. A single `(expr)` is still an ordinary parenthesized expression. The explicit `ROW(e1, e2, ...)` constructor is the same row value as the tuple `(e1, e2, ...)` and lowers to it identically; at least two elements are required, as in MySQL. As with a literal tuple, a string component compares with the engine's binary collation rather than MySQL's case-insensitive default (see the collation note below). |
...
| `IS [NOT] {NULL \| UNKNOWN \| TRUE \| FALSE}` | ✅ | `IS [NOT] NULL` maps to the engine's predicate; `UNKNOWN` is a synonym for `NULL`. The boolean tests (`IS TRUE`/`IS FALSE`) treat any non-zero, non-NULL value as true, lowered to `coalesce(x <> 0, 0)` / `coalesce(x = 0, 0)` (the `IS NOT` forms flip the comparison and default to 1), so they return 1/0 and never NULL, as in MySQL. All bind at the comparison precedence and may follow a full comparison without parentheses and chain — `1 = 1 IS TRUE`, `x IN (...) IS FALSE`, `NULL = 1 IS NULL`, `a IS NULL IS TRUE`. |
| Arithmetic `+` `-` `*`                 | ✅     | The binary operators, plus **unary** `-` / `+` on any expression (`-a`, `-ABS(x)`, `-(a + 1)`, `ABS(-a)`, `ORDER BY -a`). Unary minus binds tightly (`-a * b` is `(-a) * b`); a signed numeric literal (`-5`) is folded into the literal, and a unary sign on a literal folds too (`- -5` → `5`) rather than negating an already-signed literal text (which the engine cannot re-parse). |
| `[NOT] IN (value list)`                | 🚧     | The operand is wrapped in `COLLATE NOCASE` so membership folds ASCII case like MySQL's default collation (`'B' IN ('a', 'b')` is `1`); inert for a numeric operand, and a row-value operand (`(a, b) IN (...)`) is left as-is (a tuple takes no collation, and the comparison is element-wise). Includes the empty list: `x IN ()` folds to `0` and `x NOT IN ()` to `1` (MySQL semantics), since the engine has no empty-list `IN`. |
| `[NOT] BETWEEN a AND b`                | 🚧     | The operand is wrapped in `COLLATE NOCASE` so a string range folds ASCII case like MySQL's default collation (`'M' BETWEEN 'a' AND 'z'` is `1`); inert for a numeric operand. |
...
| `CASE` (searched and simple forms)     | 🚧     | `CASE WHEN ... THEN ... [ELSE ...] END` (searched) and `CASE expr WHEN value ... END` (simple). The simple form compares the operand to each `WHEN` value, which MySQL does under the default (case-insensitive) collation; the operand is wrapped in `COLLATE NOCASE` so a string comparison folds ASCII case like MySQL (`CASE 'b' WHEN 'B' THEN …` matches), and the wrap is inert for a numeric operand. The operand is evaluated once (safe for a non-deterministic operand like `RAND()`). As with the other `_ci` lowerings, only ASCII case folds, and a number compared to a non-numeric string is not coerced. |
...
| `CAST(expr AS type)`                   | 🚧     | Real cast syntax (not a function). Targets map onto engine affinity: `CHAR`→text, `SIGNED`/`UNSIGNED`→integer, `DOUBLE`/`FLOAT`/`REAL`→real, `BINARY`→blob; `DECIMAL`/`DEC` rounds (below). An integer cast (`SIGNED`/`UNSIGNED`) matches MySQL's argument-type-dependent rounding via a runtime `typeof` guard: a numeric value is **rounded** (`CAST(3.7 AS SIGNED)` → `4`) while a string is parsed by its leading integer (`CAST('12.9' AS UNSIGNED)` → `12`). One residual divergence: a `DOUBLE` value exactly at `.5` rounds half-to-even on MySQL but half-away-from-zero here (the engine's `round`), e.g. a `DOUBLE` `2.5` → `2` on MySQL vs `3` here (a `DECIMAL`/literal `2.5` is `3` on both). A `CHAR(n)` length **truncates** the text to `n` characters — lowered to `substr(CAST(x AS CHAR), 1, n)`, so `CAST('abcdef' AS CHAR(3))` is `abc` and `CAST(12345 AS CHAR(3))` is `123`; a shorter value is not padded (`CAST('ab' AS CHAR(5))` stays `ab`), `CHAR(0)` is the empty string, NULL propagates, and a bare `CHAR` (no length) never truncates. `UNSIGNED` is not distinguished from `SIGNED`. A `DECIMAL`/`DEC` target **rounds** the value to its scale `D` (default `0`, since bare `DECIMAL` is `DECIMAL(10,0)`): `D = 0` rounds to an integer (`CAST(123.456 AS DECIMAL)` → `123`, half away from zero; a leading-numeric string is parsed and a non-numeric one is `0`), and `D > 0` keeps `D` places (`CAST(123.456 AS DECIMAL(10,2))` → `123.46`). The width `M` is not enforced, and a `D > 0` result is not padded with trailing zeros (`CAST(100 AS DECIMAL(10,2))` renders `100` here, not MySQL's `100.00`). A `DATE`/`DATETIME`/`TIME` target is lowered to the engine's `date()`/`datetime()`/`time()`, which reformat a string value to MySQL's `YYYY-MM-DD` / `YYYY-MM-DD HH:MM:SS` / `HH:MM:SS` (an unparseable value is NULL); two edges diverge — a fractional-seconds precision (`DATETIME(6)`) parses but is dropped (no sub-second precision), and the numeric `YYYYMMDD` form (`CAST(20200615 AS DATE)`) is NULL rather than a date. A `JSON` target is rejected. |
...
| Charset introducers (`_utf8mb4'x'`, `N'x'`) | 🚧 | A string literal may carry a charset introducer — an underscore-prefixed charset name (`_utf8mb4`, `_latin1`, `_binary`, …) or the national-character `N`/`n`. The engine is single-charset, so the introducer is **dropped** and the string is used verbatim (`_utf8mb4'hello'` is `hello`, `HEX(_binary'A')` is `41`). The introducer applies only directly before a string, so an identifier named `N` or `_col`, and `N` used as an alias, are unaffected. Two minor divergences: an unknown charset name is accepted rather than rejected, and `_binary`'s case-sensitivity is not forced (the engine compares `BINARY` by default, so a bare comparison matches, but a value placed in a case-folding context — e.g. an `IN` list — folds case where MySQL's `_binary` would not). |
| Adjacent string literals (`'a' 'b'`) | ✅ | String literals adjacent in the source are concatenated into one literal — `'a' 'b' 'c'` is `'abc'` — across whitespace, newlines, and comments, as in MySQL (a SQL-standard quirk). The decoded text is joined, so an escaped quote in a piece is preserved (`'a''b' 'c'` is `a'bc`), and a charset-introduced literal (`_utf8mb4'a' 'b'`) concatenates the same way. A following non-string token (an `AS`/bare alias, an operator, a comma) ends the run, so `'a' x` stays `'a'` aliased `x`. |
| Version-gated executable comments (`/*!##### ... */`) | 🚧 | A `/*! ... */` comment (optionally version-gated, `/*!40101 ... */`) has its contents **executed as SQL** when the gate is at most the reported server version (`8.0` → `80000`), rather than discarded like a plain `/* ... */`. So `/*!40101 SET NAMES utf8mb4 */` runs the `SET`, `SELECT /*!40001 SQL_NO_CACHE */ 1` keeps the hint, and a wholly wrapped statement (`/*!40100 CREATE TABLE ... */`) is no longer empty input — the patterns mysqldump output and WordPress/plugin migrations rely on. A gate above the server version is skipped like an ordinary comment. The markers are recognized lexically, so an identical sequence inside a string literal is untouched. The session statements the server dispatches by keyword *before* parsing (`SET NAMES`, `SET @@sql_mode`, `SHOW`, `SELECT @@var`) are also handled when prefixed or wholly wrapped by a comment — `/* c */ SET NAMES utf8mb4`, `-- x`⏎`SET ...`, and mysqldump's `/*!40101 SET NAMES utf8mb4 */` are stripped/unwrapped before dispatch. One limitation: a nested plain `/* ... */` comment **inside** an executable comment is not supported (pathological; absent from real dumps). |
...
| Bit-value literals (`0b101`, `b'101'`) | 🚧     | Lexed into the engine's blob literal, mirroring the hex literals: the binary digits are packed into bytes, left-padded to a whole number of bytes as MySQL does (`b'1000001'` is the byte `0x41` = `'A'`, `b'101000001'` is two bytes `0x0141`), so `0b101` and `b'101'`/`B'101'` produce the same blob. They evaluate as those **bytes** in a string/binary context (display, `HEX()`, `LENGTH()`, column storage, blob comparison — `0b1000001 = 0x41` is `1`), matching MySQL. The same numeric/bitwise divergence as hex literals applies: MySQL coerces a bit literal to its integer value while the engine's blob coerces to `0` (`0b101 + 0` is `5` on MySQL, `0` here). |
| `[NOT] IN (SELECT ...)`                | 🚧     | Uncorrelated subquery in `IN`/`NOT IN`; the operand is wrapped in `COLLATE NOCASE` (as for the value-list form) so a string membership test folds ASCII case like MySQL, inert for a numeric operand. |
...
| `CHAR`                                 | 🚧     | Builds a string from integer character codes. Each code becomes `CASE WHEN code IS NULL THEN '' ELSE char(code) END` and the pieces are concatenated, so a **NULL code is skipped** (`CHAR(65, NULL, 66)` → `AB`), matching MySQL. Each code is coerced to an integer like `CAST(... AS SIGNED)`, so a numeric code rounds (`CHAR(65.9)` → `B`) and a string parses its leading integer (`CHAR('66')` → `B`), as in MySQL. Exact for the common ASCII / control-character codes (`CHAR(10)`, `CHAR(72, 73)`→`HI`). One divergence: for codes above 127 MySQL emits raw bytes (a number can span several) while the engine emits one UTF-8 code point. An optional trailing `USING charset` clause is parsed and ignored (the engine always builds from Unicode code points, matching MySQL's default `utf8mb4`). |
...
| `DATE_ADD` / `DATE_SUB` (`INTERVAL n unit`) | 🚧 | Lowered to the engine's `datetime(x, '±n unit')` modifier. A simple `unit` ∈ `DAY`/`WEEK`/`MONTH`/`QUARTER`/`YEAR`/`HOUR`/`MINUTE`/`SECOND` (`WEEK`→7 days, `QUARTER`→3 months) takes an integer-literal value. A **compound** unit — `YEAR_MONTH`, `DAY_HOUR`, `DAY_MINUTE`, `DAY_SECOND`, `HOUR_MINUTE`, `HOUR_SECOND`, `MINUTE_SECOND` — takes a multi-field string literal (`'1:30'`, `'2-3'`, `'1 2:3:4'`; WordPress's GMT-offset upgrade uses `INTERVAL '<h>:<m>' HOUR_MINUTE`), split into one field per engine unit and lowered to a multi-modifier `datetime(...)`; a leading `-` (and `DATE_SUB`) negates every field. Matches MySQL for DATETIME arguments, including `MONTH`/`QUARTER`/`YEAR` arithmetic that overflows a shorter month: the result is clamped to the month's last day (`Jan 31 + 1 month` → `Feb 28`, keeping the time), rather than rolling into the next month — implemented by stepping back the overflowed days when the result's day-of-month is smaller than the input's; a magnitude beyond representable dates is not handled. The result keeps MySQL's DATE-vs-DATETIME type: a date-unit step (`DAY`/`WEEK`/`MONTH`/`QUARTER`/`YEAR`) on an argument with no time of day returns a **DATE** (`DATE_ADD('2026-06-15', INTERVAL 1 DAY)` → `2026-06-16`, no `00:00:00`), while a time-unit step, or an argument that already carries a time, returns a DATETIME — decided at runtime by whether the argument's text holds a time component (a `:`), so a `DATE` column yields a DATE and a `DATETIME` column keeps its time. A compound unit always returns a DATETIME, and its `YEAR_MONTH` overflow case is not clamped. The same lowering also backs the `+`/`-` operator forms, where the interval may appear on **either** side of `+` — `INTERVAL n unit + date` is equivalent to the postfix `date + INTERVAL n unit` — while a leading interval must be followed by `+` (a standalone `INTERVAL n unit`, or the `INTERVAL n unit - date` prefix, is rejected, as in MySQL). |
| `TIMESTAMPADD(unit, n, dt)`             | 🚧     | Shifts `dt` by `n` units — the **same lowering** as `DATE_ADD(dt, INTERVAL n unit)` (same units, same integer-literal requirement, same DATE-vs-DATETIME return type, and the same month-end clamp on a `MONTH`/`QUARTER`/`YEAR` step that overflows a shorter month). The counterpart to `TIMESTAMPDIFF`. |
...
| `ADDDATE` / `SUBDATE`                   | 🚧     | The `INTERVAL` form is identical to `DATE_ADD`/`DATE_SUB`. The integer-days form (`ADDDATE(d, n)`) shifts by `n` whole days, lowered to `datetime(d, printf('%+d days', ±n))` with a NULL-day guard; `n` may be any expression. A whole-day step is a date unit, so it returns the same DATE-vs-DATETIME type as `DATE_ADD` (a bare DATE yields a DATE, a value with a time keeps it). |
| `DATE_FORMAT(x, fmt)`                   | 🚧     | Lowered to `strftime()` for the directly-translatable specifiers (`%Y %m %d %H` pass through; `%i`→`%M`, `%s`→`%S`; `%j` day-of-year, `%w` weekday-number, `%U` Sunday-first week pass through; `%v`→`%V` ISO week; `%x`→`%G` ISO week-year; `%T`→`%H:%M:%S`; `%%` literal), and the name specifiers `%M` (month name), `%b` (abbreviated month), `%W` (weekday name), `%a` (abbreviated weekday) are expanded to `CASE` lookups and concatenated (English names only), the no-leading-zero numeric specifiers `%e` (day), `%c` (month), `%k` (hour) become integer casts of the strftime code, the week numbers `%u` (Monday-first, mode 1) and `%V` (Sunday-first, mode 2) reuse the `WEEK()` mode lowering zero-padded to two digits (`%U`/`%v` keep their direct strftime form), the two week-years pair with their week scheme — `%x` (Monday-first ISO) → strftime `%G`, and `%X` (Sunday-first, the `%V` companion) → a `CASE` that carries a January date in the previous year's last week back a year — and the 12-hour clock `%l` (no pad) / `%h`/`%I` (padded) / `%r` (`hh:mm:ss AM/PM`), meridiem `%p` (AM/PM), the two-digit year `%y` (last two digits of `%Y`), and the day-with-ordinal-suffix `%D` (`1st`, `2nd`, …) become `CASE` / `strftime` expressions. The format must be a **parse-time string constant** — a string literal, a run of adjacent literals, or a `GET_FORMAT(...)` call (so `DATE_FORMAT(d, GET_FORMAT(DATE, 'EUR'))` works); a `GET_FORMAT` that resolves to NULL makes the whole call NULL. Only `%f` (microseconds — no sub-second precision) is rejected rather than silently mistranslated. The pieces are concatenated as a *balanced* `||` tree, so a long format packed with the `CASE`-heavy clock specifiers (e.g. `%h %I %l %p %r %T %k`) stays shallow instead of building a deep left-linear chain that overflows the engine's expression-evaluator stack. |
| `TIME_FORMAT(x, fmt)`                   | 🚧     | Shares the `DATE_FORMAT` lowering. For a time-only format (`%H %i %s %h %I %l %p %k %T %r`, literal text, …) it matches MySQL exactly, since those specifiers read only the time part — of either a `TIME` or a `DATETIME` argument. Two divergences from MySQL's stricter `TIME_FORMAT`: a *date* specifier (`%Y`, `%m`, `%W`, …) is evaluated here rather than returning NULL, and a `TIME` value outside `00:00:00..23:59:59` (MySQL allows up to 838 hours) is not represented. The format is the same parse-time string constant `DATE_FORMAT` takes (a literal, adjacent literals, or `GET_FORMAT(...)`), and the specifiers `DATE_FORMAT` rejects (`%f`) are rejected here too. |
| `GET_FORMAT(type, locale)`              | ✅     | The constant `DATE_FORMAT`-style format string for a `(type, locale)` pair, resolved at parse time to the matching string literal. `type` is a bare keyword — `DATE`, `TIME`, `DATETIME`, or its synonym `TIMESTAMP`; `locale` is a string literal matched case-insensitively against the five names `EUR`, `USA`, `JIS`, `ISO`, `INTERNAL`. An unrecognized locale yields NULL (as in MySQL); an unrecognized type keyword is rejected. The MySQL quirks are reproduced verbatim (`USA` date is month-first `%m.%d.%Y`, but the `USA` *datetime* keeps the ISO date with a dotted time, identical to `EUR`). Since the result is a parse-time literal, it pairs directly with `DATE_FORMAT`/`TIME_FORMAT`/`FROM_UNIXTIME` (`DATE_FORMAT(d, GET_FORMAT(DATE, 'EUR'))`), its intended use. |
...
| `VERSION` / `DATABASE` / `SCHEMA` / `CONNECTION_ID` / `USER` / `CURRENT_USER` / `SESSION_USER` / `SYSTEM_USER` | 🚧 | Introspection functions, usable in any expression (not just as a standalone `SELECT`). Most fold to fixed placeholder values matching the server's standalone-query answers (`VERSION()`→`8.0.0-turso`, `USER()`→`root@localhost`, `CONNECTION_ID()`→`1`) and are not the real per-connection values. `DATABASE()`/`SCHEMA()`, however, **do** report the connection's current database — the name the client selected in the handshake or with a later `USE` — threaded in from the server session (`NULL` when none is selected), matching MySQL. `CURRENT_USER` is also accepted without parentheses (the SQL-standard niladic form); the others require parentheses, as in MySQL. |
