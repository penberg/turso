...
| `TIMESTAMPADD(unit, n, dt)`             | 🚧     | Shifts `dt` by `n` units — the same lowering as `DATE_ADD(dt, INTERVAL n unit)` (same units, same integer-literal requirement, same DATE-vs-DATETIME return type). The counterpart to `TIMESTAMPDIFF`. Unlike `DATE_ADD`, a `MONTH`/`QUARTER`/`YEAR` step is not month-end-clamped here. |
