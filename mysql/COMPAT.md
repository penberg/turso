...
| `STRCMP(a, b)`                         | 🚧     | Returns -1 / 0 / 1 for `a < b` / `a = b` / `a > b`, and NULL if either argument is NULL — lowered to a `CASE` over the comparison operators. The comparison uses the engine's binary (case-sensitive) collation, so it diverges from MySQL's case-insensitive default for strings differing only in case (`STRCMP('a', 'A')` is `1` here vs `0` on MySQL). |
...
| `agg(...) OVER (...)` (window aggregate) | 🚧 | An aggregate (`SUM`/`COUNT`/`MIN`/`MAX`/`AVG`, including `COUNT(*)`) with an `OVER ( [PARTITION BY ...] [ORDER BY ...] )` clause is evaluated by the engine: `OVER ()` aggregates the whole result, `PARTITION BY` aggregates per group, and `ORDER BY` gives a running aggregate under MySQL's default frame. An explicit frame (`ROWS`/`RANGE`/`GROUPS ...`) and a named window (`OVER w` with a `WINDOW` clause) are rejected, and the dedicated window functions (`ROW_NUMBER`, `RANK`, `LAG`, …) are not available. |
...
| `WEEK(d[, mode])`                       | 🚧     | The week number, lowered to a `strftime` week. The `mode` (default 0) selects the numbering scheme; four are supported: 0→`%U`, 3→`%V` (ISO), 5→`%W`, and 1 (Monday-first, 0–53, week 1 = first week with >3 days — which `WP_Date_Query` uses when the week starts Monday) → the ISO week with a year-boundary correction (0 in a previous-year partial week, 53 in a next-year partial week), verified against MySQL 8.4 for 2016–2031. Modes 2/4/6/7 (whose 1–53 / Sunday-based boundary numbering has no clean strftime form) are rejected. `WEEKOFYEAR(d)` is `WEEK(d, 3)`. |
